...
function html(body = `<script>${script}</script>`, csp = policy, pathname = '/') {
  return `<!doctype html><html><head><meta http-equiv="Content-Security-Policy" content="${csp}"><link rel="canonical" href="https://test.pages.dev${pathname}"></head><body>${body}</body></html>`;
}
